...
- **`switch_harness` can't reach the Phase 3 harnesses.** Fixed in `de733ed` (Plan BM P5): case-insensitive kind lookup + updated schema enum, so `"codexcloud"`/`"claudeRemote"` now match.
- **Service-layer gating (BK P0 rider):** `OpenClawBridge.agentRequest`/`AgentSessionService.dispatch` had no `agentModeEnabled` guard at the service layer. Closed by `e75c0b4` (Plan BK P0).
- **Custom/preset harness hygiene:** `http://` acceptance and the unencoded `{id}` template substitution. Closed in `de733ed` (Plan BM P5): https required/warned, run id percent-encoded.
- **`confirm` is self-approvable.** Closed by `65bbcee` (Plan BN P1): `confirm` now routes through a user-distinct check via `RemoteActionConsentView` — a phone-side consent card, not an in-lens HUD confirm — which also serves as the shared consent affordance Phase 4 wanted.
