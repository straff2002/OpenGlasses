...
**Related findings:** F08/F09/F11/F20–F23 in [the assessment](EQ-compliance-assessment.md). Shared implementation: W01/W03/W04/W05/W06/W07/W08 in [the roadmap](EU-remediation-roadmap.md).
...
- **AI-04's corpus and gate half** is [`SafetyEvalCorpus`](../../OpenGlassesTests/Fixtures/SafetyEvalCorpus/README.md) with `SafetyEvalHarness`, `SafetyEvalGateTests` and [`safety-eval.yml`](../../.github/workflows/safety-eval.yml). 52 synthetic cases across the four safety-output verticals, run through the real handling path, reporting false-negative, overconfidence, abstention-when-required and escalation-present rates per vertical, risk class and subgroup — and no combined accuracy number, in line with section 7's rule against a universal figure. A critical- or high-risk breach blocks the pull request. `PromptVersionRegistryTests` ties the results to the instruction versions they were measured against, so a prompt change forces a re-evaluation. What the package still asks for and does not have: the approved thresholds (these are proposed floors and say so), expert labelling, a corpus of real frames, and the model-side evaluation that would make a provider or model change measurable.
...
The **hazard vision** and **health advice** families above have a first, partial implementation: the safety evaluation corpus at [`OpenGlassesTests/Fixtures/SafetyEvalCorpus`](../../OpenGlassesTests/Fixtures/SafetyEvalCorpus/README.md), with its gate in `SafetyEvalGateTests` and [`.github/workflows/safety-eval.yml`](../../.github/workflows/safety-eval.yml). It covers the occluded-hazard, poor-lighting, motion-blur, incomplete-view and unrelated-scene scenarios for hazard vision, and the unknown-medicine, incomplete-vault, multilingual and emergency-symptom scenarios for health advice, including the case this plan cares most about — a deterministic warning the model is actively contradicting. It measures false negatives, unsupported certainty and abstention, which is what those rows ask for. Three things it is not: the scoring is against authored expectations rather than expert labels; the inputs are synthetic fixtures and supplied quality indicators rather than captured frames; and the thresholds are proposed rather than approved, so the "minimum gates approved before running tests" condition in the hazard-vision row is **not met**. The other five families have no corpus. A prompt or schema change is now tied to the corpus version it was evaluated at, through the instruction digests in `prompt-digests.json`.
