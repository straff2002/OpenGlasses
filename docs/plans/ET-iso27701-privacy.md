...
| clinicalTranscripts | `HIPAAComplianceService` | clinical | thirdPartySubject | complete | yes | clinical retention days, whatever the mode; disabled at zero | `HIPAAComplianceService.secureDelete` | none — transcripts are filed by session, not by patient |
