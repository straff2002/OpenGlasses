...
| F11 | Every tool call is classified by effect, and anything above a read is held for an approval bound to the seam, the tool, its definition digest, the canonical argument digest and the approval session — single use, with a distinct refusal class for replay, changed arguments, a different tool, a different server, a changed definition, expiry and cross-session use. The digest last accepted per server is persisted so changed definitions re-quarantine, a quarantined description is replaced with app-authored text before the model sees it, and the outbound egress screen now runs before anybody is asked to approve anything. | The 67 focused approval-binding, effect-class, re-review and consent tests passed inside a full run of 4,943 tests (13 skipped, 0 failures) on the iOS 27 simulator, followed by a clean `-configuration Release` build. | Adversarial end-to-end exercise against a live hostile MCP server, the user-authored HTTP tool route and the gateway route; a wearer-facing re-review action for a quarantined definition; better-than-unreviewed classification for the gateway and custom HTTP seams; and a decision on whether native writes to the wearer's own stores should also be bound. |
