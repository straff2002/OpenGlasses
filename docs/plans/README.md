...
| W Presence-Aware Agent Throttle | 🚧 Core shipped (this PR) — `EngagementMode`/`Autonomy`/`ThrottleDecision` + pure `ThrottlePolicy.decide` (table + `minMode` floor); `PresenceEvaluator` signal fusion + `ModeDebouncer` (rise-instant/drop-dwell) + injectable `PresenceMonitor`; 21 tests. Deferred: live loop/Plan S/app wiring + held-recommendation surfacing. |
