...
| W Presence-Aware Agent Throttle | ✅ Shipped — core (`ThrottlePolicy`/`PresenceMonitor`, 21 tests) + live integration: `LoopThrottle` gating `LiveCoachService`/`ProactiveAlertService`; presence autonomy ceiling in `SafetySupervisor` (acting tools held when idle/away); `HeldRecommendationStore` surfaced on re-engagement (TTS+HUD); `PresenceMonitor` constructed + signal-wired in AppState. +15 tests (667 total). Deferred: optional CoreMotion signal; continuous-caption throttle. |
