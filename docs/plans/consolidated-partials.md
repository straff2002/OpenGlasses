...
|---|---|---|
| [BH](BH-gateway-remote-invoke.md) | Two open safety gaps: reclass `getTranscript` out of the default-on `.observe` class; attribute remote `speak` text to its source | Both re-verified in `RemoteGlassesCommand` and `RemoteCommandExecutor`. The origin-aware policy/audit refactor the doc still calls "scheduled" shipped as BN P2 |
