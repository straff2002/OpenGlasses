...
| [EO](EO-hevc-glasses-stream.md) | A stall recovery that exhausts its tiers is not picked up by the reconnect ladder | The stop-during-warmup race is **closed** by [EW](EW-session-resource-cleanup.md) (2026-09-15). The recovery/reconnect handoff is a stream-recovery decision, so [BR](BR-realtime-and-stream-hardening.md)'s |
...
| [EC](EC-ui-localization.md) | P1 items 1–4 (`CapabilityCatalog` copy to `LocalizedStringResource`; `String`-only `OGChip`/`OGStatusPill`/`OGHeroDeviceCard`/`OGDiscoverCard`; runtime-composed sentences; plural variants), then P2's specifier-parity and coverage-floor guardrail tests and P3's catalogs | The design-kit `LocalizedStringKey` prerequisite merged, and [#483](https://github.com/straff2002/OpenGlasses/pull/483) (2026-09-15) disfavoured its verbatim overloads so literal copy is extracted again; catalog syncs [#481](https://github.com/straff2002/OpenGlasses/pull/481)/[#483](https://github.com/straff2002/OpenGlasses/pull/483) added keys but no translations |
...
| Plan | Outstanding item | Notes |
|---|---|---|
| [EW](EW-session-resource-cleanup.md) | Device smoke only — network flap, folded/doffed glasses, background/lock, stop during cold start, residual resource counts | Core shipped 2026-09-15: the [audit table](EW-resource-audit.md), 15 headless exit tests and 7 closed gaps, including EO's stop-during-warmup race. The realtime managers are not constructible headless, so their exit rows are owed hardware |
...
| [DH](DH-local-gemma-keyless-tier.md) | P1 + P2; P3's headroom correction (`MemoryHeadroom`, photo-turn refusal, pre-resize cap) | P3's full vision-wiring measurement — decode latency and Metal contention against on-device TTS; P4's performance matrix (tok/s, first-token latency, battery, thermal) across phones, which sets P2's real thresholds; and [#465](https://github.com/straff2002/OpenGlasses/pull/465)'s smaller-model offer on a 6 GB phone | Several phones, oldest included |
...
| [ET](ET-iso27701-privacy.md) — Medical Compliance data at rest | Recording artefacts (audio and video recordings, both transcript copies, the sessions list) get `.completeUnlessOpen` + backup exclusion in the mode, in-progress recordings start in a folder carrying the class, and enabling the mode sweeps existing files; audit log and exports keep `.complete` ([#486](https://github.com/straff2002/OpenGlasses/pull/486)) | The simulator reports no protection class, so the class is unverified: with the mode on, record audio-only then video, lock for more than 10 s, stop by voice or from the glasses, unlock and confirm each recording plays and its transcript opens; confirm a file created in the protected folder inherits the class | A phone, Medical Compliance on, glasses for the locked stop |
...
| [EG](EG-vault-packs.md) | P1+P2 (manifest, signature reuse, registry); a signed empty `vaultpacks/catalog.json`, published live since [#485](https://github.com/straff2002/OpenGlasses/pull/485) | Per-pack App Store Connect products, and the key holder signing each pack into the catalog | The signing key holder; App Store Connect |
...
| [L](L-webrtc-expert-transport.md) · [M](M-webrtc-infra-and-audio.md) | App-side WebRTC + audio coordinator; M1/M2 reference impls | Deploy the signaling relay and TURN (**on-demand only** — the meeting-link path covers remote today, and room-token auth is the real gate, in §A); host the expert web client. The MJPEG path needs a relay too: since [#471](https://github.com/straff2002/OpenGlasses/pull/471) the app ships none, so browser streaming waits for one entered in Settings | A self-host or compliance customer |
