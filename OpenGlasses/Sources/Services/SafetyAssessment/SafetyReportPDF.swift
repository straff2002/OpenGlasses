...
enum SafetyReportPDF {
    ...
    static func data(for report: SafetyReport) -> Data {
        ...
        format.documentInfo = report.provenance?.pdfDocumentInfo ?? [
            kCGPDFContextCreator as String: "OpenGlasses — AI-generated",
    ...
    /// The page body, for tests that need to read what the document actually says.
    static func bodyTextForTesting(_ report: SafetyReport) -> String { bodyText(report) }
    ...
    private static func bodyText(_ report: SafetyReport) -> String {
        ...
        lines.append("")
        lines += AssessmentPresentation.caveatLines(kind: "safety_assessment",
                                                    limitations: report.limitations,
                                                    certainty: nil)
        lines.append("")
        lines.append("Advisory only — verify on site. Not a certified safety inspection.")
        lines.append(report.provenance?.footerLine
            ?? "AI-generated by OpenGlasses. The model that produced it was not recorded.")
